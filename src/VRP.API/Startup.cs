        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            var config = new MapperConfiguration(cfg =>
            {
                ...
                cfg.CreateMap<CharacterModel, CharacterDto>()
                    .ForMember(
                        accountDto => accountDto.PlayedTime,
                        opt => opt.ResolveUsing((model, dto) => $"{model.PlayedTime.Hours} h {model.PlayedTime.Minutes} min"))
                    .ForMember(
                        accountDto => accountDto.TodayPlayedTime,
                        opt => opt.ResolveUsing((model, dto) => $"{model.TodayPlayedTime.Hours} h {model.TodayPlayedTime.Minutes} min"))
                    .PreserveReferences()
                    .EqualityComparison((model, dto) => model.Id == dto.Id)
                    .ForAllMembers(opts => opts.Condition((src, dest, srcMember) => srcMember != null));
                ...
                cfg.CreateMap<CharacterDto, CharacterModel>()
                    .ForMember(
                        accountModel => accountModel.PlayedTime,
                        opt => opt.Ignore())
                    .ForMember(
                        accountModel => accountModel.TodayPlayedTime,
                        opt => opt.Ignore())
                    .PreserveReferences()
                    .EqualityComparison((dto, model) => model.Id == dto.Id)
                    .ForAllMembers(opts => opts.Condition((src, dest, srcMember) => srcMember != null));
